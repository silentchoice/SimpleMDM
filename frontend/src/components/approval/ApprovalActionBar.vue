...
import { approveApprovalTask, rejectApprovalTask, type ApprovalStatus, type ApprovalTaskType } from '../../api/approval'
import type { ApiError } from '../../types'

const props = defineProps<{ taskId: number, status: ApprovalStatus, taskType: ApprovalTaskType }>()
const { t } = useI18n()
...
async function run(action: () => Promise<void>, event: 'approved' | 'rejected'): Promise<void> {
  ...
  try {
    await action()
    if (event === 'approved') emit('approved')
    else emit('rejected')
  } catch (cause) {
...
function approve(): Promise<void> {
  return run(() => approveApprovalTask(props.taskId, props.taskType, comment.value.trim() || undefined), 'approved')
}

function reject(): Promise<void> {
  if (!reason.value.trim()) {
    error.value = t('approval.actions.rejectionRequired')
    return Promise.resolve()
  }
  return run(() => rejectApprovalTask(props.taskId, props.taskType, reason.value.trim()), 'rejected')
}
</script>
