<template>
  <el-card>
    ...
    <el-form label-width="120px">
      ...
      <el-button v-if="canEdit" data-test="save" :disabled="loading || unavailable" type="primary" @click="save">保存</el-button>
...
function validate() {
  ...
  const invalidDecimal = fields.value.find(field => {
    ...
    return (match[2]?.length || 0) > scale || match[1].replace(/^0+/, '').length + (match[2]?.length || 0) > precision
  })
...
async function save() {
  if (!canEdit.value || unavailable.value || loading.value) return
