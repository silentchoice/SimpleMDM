...
public class FieldDefinition {
    ...
    public void apply(MetadataCommands.UpdateField command, Long actorId) {
        fieldName = command.fieldName(); dataType = command.dataType(); required = command.required();
        uniqueValue = command.uniqueValue(); searchable = command.searchable(); maxLength = command.maxLength();
        precision = command.precision(); scale = command.scale(); referenceObjectTypeId = command.referenceObjectTypeId();
        defaultValue = command.defaultValue(); validationRule = command.validationRule(); sortOrder = command.sortOrder();
        updatedBy = actorId;
    }
