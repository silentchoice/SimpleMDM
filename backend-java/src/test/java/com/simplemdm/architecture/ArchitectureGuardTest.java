...
class ArchitectureGuardTest {
 ...
 static List<String> violations(String path,String source){return List.of("model/MdmFieldDefinition.java","class MdmFieldDefinition","interface MdmFieldDefinition","record MdmFieldDefinition","model/SysUser.java","class SysUser","SysUserPermission","LEGACY_CURRENT_USER","MdmPersonnel","PersonnelSubService","PersonnelService","DynamicFieldService","owner_dept","mdm_personnel","dynamic_data","com.simplemdm.model.MdmFieldDefinition","PersonnelController","MdmPersonnelRepository","DynamicPersonnelDTO").stream().filter(t->path.replace('\\','/').contains(t)||source.contains(t)).map(t->path+" -> "+t).toList();}
 static List<String> frontendViolations(String path,String source){return List.of("/personnel","views/personnel","dept-fields","api/personnel","api/deptFields","router.push('/dashboard')","perm_type","scope_value").stream().filter(source::contains).map(t->path+" -> "+t).toList();}
