...
class JwtInterceptorTest {
    ...
    void authenticatesActiveUserOnlyWhenTokenSystemMatches() throws Exception {
        ...
        when(users.findWithContextById(7L)).thenReturn(Optional.of(user));
    ...
    void rejectsMissingSystemClaimMismatchedSystemAndInactiveUser() throws Exception {
        ...
        when(users.findWithContextById(7L)).thenReturn(Optional.of(active));
        ...
        when(users.findWithContextById(7L)).thenReturn(Optional.of(inactive));
        ...
        when(users.findWithContextById(7L)).thenReturn(Optional.of(disabledSystemUser));
        assertThat(interceptor.preHandle(request(jwtUtil.createToken(7L, 10L)), new MockHttpServletResponse(), new Object()))
            .isFalse();
