...
class ApprovalServiceValidationTest {
    ...
    void referenceChangeUsesReferenceColumnsAndIsRehydratedForApprovedWriter() {
        ...
        when(f.requests.findById(100L)).thenReturn(Optional.of(request));when(f.records.findBySystemIdAndObjectTypeIdAndIdAndDeletedAtIsNull(7L,8L,41L)).thenReturn(Optional.of(record));
        ...
        when(f.writer.apply(100L)).thenReturn(new RecordView(41L,7L,8L,9L,"EMP",4L));
        f.service.approve(100L,20L,3L);
        verify(f.writer).apply(100L);
    }
    ...
    private static class Fixture {
        ...
        MdmRecord recordAndPermission(){MdmRecord r=mock(MdmRecord.class);when(r.getId()).thenReturn(41L);when(r.getSystemId()).thenReturn(7L);when(r.getObjectTypeId()).thenReturn(8L);when(r.getDepartmentId()).thenReturn(9L);when(r.getVersion()).thenReturn(3L);when(records.findBySystemIdAndObjectTypeIdAndIdAndDeletedAtIsNull(7L,8L,41L)).thenReturn(Optional.of(r));when(authorization.can(anyLong(),eq("MDM_RECORD_EDIT"),eq(9L))).thenReturn(true);return r;}
